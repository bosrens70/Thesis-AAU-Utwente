# -*- coding: utf-8 -*-
"""
HDBSCAN Instance Segmentation — Sliced-PCA split for curved pipes
=================================================================
Same as hdbscan_instance_site.py but replaces the global-PCA pipe split
with a sliding-window approach: each instance is cut into short segments
along its length, PCA + 2D DBSCAN is run per segment, and consistent
sub-labels are stitched across segments.  This handles hanging cables
and curved pipes where a single PCA axis is inaccurate.

Key parameters:
    MIN_CLUSTER_SIZE —  HDBSCAN parameter: minimum number of points to form a cluster.
    MIN_SAMPLES      — Optional HDBSCAN parameter: controls how conservative the
                       algorithm is about core point density.
    MIN_INSTANCE_POINTS — instances smaller than this become noise (post-processing).
    SLICE_LENGTH     — length of each sliding-window segment (metres).
    2D eps           — DBSCAN epsilon on the cross-section plane per slice.
    2D min points    — minimum points per sub-cluster in the cross-section.
"""
...
PLY_FILE = (
    ...
    r"\Python\Thesis\Data\OpenTrench3D\Water_Area_5\Area_5_Site_22.ply"
)
...
def split_sliced_pca(labels, slice_len, eps_2d, min_pts_2d, min_split_size, target_instance):
    """Split a specific instance using sliding-window slices along the pipe axis.

    For the target instance:
      1. Fit global PCA to get the main axis and two perpendicular axes.
      2. Project onto the main axis → 1D coordinate t.
      3. Slide a window of length `slice_len` along t (50% overlap).
      4. Per slice: project points onto the global cross-section plane
         (PC2, PC3) and run DBSCAN in 2D.
      5. Stitch sub-labels across slices using an overlap graph —
         two sub-clusters in adjacent slices that share points are the
         same pipe.
    """
    ...
    for uid in [target_instance]:
        ...
        n_pts = len(inst_pts)
        if n_pts < min_pts_2d * 2:
            continue

        # Global PCA: PC1 = pipe axis, PC2/PC3 = cross-section plane
        pca_global = PCA(n_components=3)
        transformed_all = pca_global.fit_transform(inst_pts)
        t = transformed_all[:, 0]  # projection along pipe axis
        cross_all = transformed_all[:, 1:3]  # cross-section for all points

        t_min, t_max = t.min(), t.max()
        pipe_length = t_max - t_min
        if pipe_length < slice_len:
            sub = DBSCAN(eps=eps_2d, min_samples=min_pts_2d).fit_predict(cross_all)
            n_sub = len(set(sub) - {-1})
            if n_sub <= 1:
                continue
            indices = np.where(inst_mask)[0]
            for sid in sorted(set(sub) - {-1}):
                if sid == 0:
                    new_labels[indices[sub == sid]] = uid
                else:
                    new_labels[indices[sub == sid]] = next_id
                    next_id += 1
            new_labels[indices[sub == -1]] = -1
            print(f"  instance {uid}: short pipe, global split → {n_sub} sub-instances")
            continue

        # Build slices with 50% overlap
        step = slice_len * 0.5
        slice_starts = np.arange(t_min, t_max - slice_len + 1e-9, step)
        if len(slice_starts) == 0:
            slice_starts = np.array([t_min])

        slice_results = []  # list of (point_indices_in_inst, sub_labels)
        node_id = 0
        node_map = {}  # (slice_idx, sub_id) → graph node

        for si, s_start in enumerate(slice_starts):
            s_end = s_start + slice_len
            in_slice = (t >= s_start) & (t < s_end)
            slice_idx = np.where(in_slice)[0]
            if len(slice_idx) < min_pts_2d:
                slice_results.append((slice_idx, np.full(len(slice_idx), -1)))
                continue

            # Use the global cross-section axes, not a local PCA
            cross = cross_all[slice_idx]

            sub = DBSCAN(eps=eps_2d, min_samples=min_pts_2d).fit_predict(cross)
            slice_results.append((slice_idx, sub))

            for sid in set(sub) - {-1}:
                node_map[(si, sid)] = node_id
                node_id += 1

        if node_id == 0:
            continue

        # Build adjacency graph: connect nodes in adjacent slices
        # that share the same points (via overlap).
        adj = lil_matrix((node_id, node_id), dtype=bool)
        for si in range(len(slice_results) - 1):
            idx_a, sub_a = slice_results[si]
            idx_b, sub_b = slice_results[si + 1]
            ...
            set_a = set(idx_a)
            set_b = set(idx_b)
            overlap_pts = sorted(set_a & set_b)
            if not overlap_pts:
                continue

            # For each overlap point, find its sub-label in both slices
            pos_in_a = {v: i for i, v in enumerate(idx_a)}
            pos_in_b = {v: i for i, v in enumerate(idx_b)}

            for pt in overlap_pts:
                sa = sub_a[pos_in_a[pt]]
                sb = sub_b[pos_in_b[pt]]
                if sa < 0 or sb < 0:
                    continue
                na = node_map.get((si, sa))
                nb = node_map.get((si + 1, sb))
        ...
        # Map each graph node back to a component (= final pipe id)
        node_to_pipe = {node: int(comp_labels[node]) for node in range(node_id)}

        # Assign final labels: iterate slices, for each point take its
        # pipe id.  Later slices overwrite earlier ones (overlap is fine,
        # they should agree).
        pipe_labels = np.full(n_pts, -1, dtype=int)
        for si, (slice_idx, sub) in enumerate(slice_results):
            for local_i, global_i in enumerate(slice_idx):
                sid = sub[local_i]
                if sid < 0:
                    continue
                node = node_map.get((si, sid))
                if node is not None:
                    pipe_labels[global_i] = node_to_pipe[node]

        # Drop sub-pipes smaller than min_split_size
        for pid in set(pipe_labels) - {-1}:
            if (pipe_labels == pid).sum() < min_split_size:
                pipe_labels[pipe_labels == pid] = -1

        n_pipes = len(set(pipe_labels) - {-1})
        if n_pipes <= 1:
            continue

        indices = np.where(inst_mask)[0]
        pipe_ids = sorted(set(pipe_labels) - {-1})
        for rank, pid in enumerate(pipe_ids):
            pmask = pipe_labels == pid
            if rank == 0:
                new_labels[indices[pmask]] = uid
            else:
                new_labels[indices[pmask]] = next_id
                next_id += 1
        new_labels[indices[pipe_labels == -1]] = -1
        print(f"  instance {uid} split into {n_pipes} sub-instances "
              f"({len(slice_starts)} slices)")
...
class InstanceViewer:
    ...
    def __init__(self):
        ...
        self.window = gui.Application.instance.create_window(
            f"HDBSCAN Instance Viewer (sliced PCA) — {ply_path.name} ", 1440, 900)
        ...
        split_title = gui.Label("Split pipes (sliced PCA)")
        ...
        panel.add_child(gui.Label("Slice length (m):"))
        self._slice_edit = gui.NumberEdit(gui.NumberEdit.DOUBLE)
        self._slice_edit.double_value = 0.5
        self._slice_edit.set_limits(0.05, 10.0)
        panel.add_child(self._slice_edit)
    ...
    def _on_split(self):
        ...
        slice_len = self._slice_edit.double_value
        ...
        print(f"\nSplitting instance {target} — sliced PCA (slice={slice_len:.2f}m, "
              f"eps_2d={eps_2d:.3f}, min_pts_2d={mp2d}, min_split_size={mss}) ...")

        labels, uniq, n_inst, n_noise = split_sliced_pca(
            self._cur_labels, slice_len, eps_2d, mp2d, mss, target)
        colors, pal = make_colors(labels, uniq, n_inst)
