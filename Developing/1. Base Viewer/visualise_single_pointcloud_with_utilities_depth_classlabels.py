...
if len(picked_indices) == 0:
    ...
    for i, idx in enumerate(picked_indices):
        p = pts[idx]
        print(f"    [{i+1}]  index {idx:>8,}  ->  " # print the index of the point
              f"X={p[0]:.3f}  Y={p[1]:.3f}  Z={p[2]:.3f}") # print the X, Y, Z coordinates of the point
...
_depth_stats = {"estimated": 0, "fallback_feature_mean": 0, "fallback_global": 0} # initialize the depth estimation counters

# ─────────────────────────────────────────────────────────────────────────────
# 4.  Geometry helpers
# ─────────────────────────────────────────────────────────────────────────────
def segment_to_cylinder(p1, p2, radius, color, resolution=12): # define a function to convert a line segment to a cylinder
    vec    = p2 - p1
    length = np.linalg.norm(vec)
    if length < 1e-6: # if the length of the segment is less than 1e-6, return None
        return None

    cyl = o3d.geometry.TriangleMesh.create_cylinder( # create a cylinder
        radius=radius, height=length, resolution=resolution, split=1
    )
    z_axis    = np.array([0.0, 0.0, 1.0]) # define the z-axis
    direction = vec / length # define the direction of the segment
    cross     = np.cross(z_axis, direction)
    cross_norm = np.linalg.norm(cross) # define the norm of the cross product
    dot        = np.dot(z_axis, direction) # define the dot product of the z-axis and the direction

    if cross_norm > 1e-6:
        axis  = cross / cross_norm # define the axis of the cylinder
        angle = np.arctan2(cross_norm, dot)
        R = o3d.geometry.get_rotation_matrix_from_axis_angle(axis * angle) # define the rotation matrix
...
if all_pipe_meshes:
    ...
    # Wireframe version of the merged pipe mesh (for the wireframe toggle)
    combined_pipe_wire = o3d.geometry.LineSet.create_from_triangle_mesh(combined_pipe)
...
# `defaultLit` shader.  Open3D does not ship with a true Eye-Dome-Lighting
# (EDL) shader, so the closest equivalent is lit points + SSAO post-processing,
# both enabled below when the scene is built.
try:
...
bbox_wire_pts = np.stack([
    _crop_cx_local + CROP_RADIUS * np.cos(_theta),
    _crop_cy_local + CROP_RADIUS * np.sin(_theta),
...
# Add pipes (filled) + wireframe version; only one is visible at a time
...
# ── View options (origin axis, pipe wireframe) ─────────────────────────────
...
pipe_wire_cb = gui.Checkbox("Wireframe pipes")
pipe_wire_cb.checked = False
if combined_pipe_wire is None:
    pipe_wire_cb.enabled = False


def _on_pipe_wire_toggle(checked):
    pipe_wireframe_active[0] = checked
    if combined_pipe is not None:
        scene_widget.scene.show_geometry(PIPE_GEOM, not checked)
    if combined_pipe_wire is not None:
        scene_widget.scene.show_geometry(PIPE_WIRE_GEOM, checked)
    window.post_redraw()


pipe_wire_cb.set_on_checked(_on_pipe_wire_toggle)
panel.add_child(pipe_wire_cb)
panel.add_fixed(int(0.8 * em))
...
def _apply_opacity(val: float):
    ...
    if combined_pipe is not None:
        ...
        # Re-adding resets visibility — honour the current wireframe toggle
        scene_widget.scene.show_geometry(PIPE_GEOM, not pipe_wireframe_active[0])
    if combined_comp is not None:
...
bbox_row.add_child(gui.Label(f"Crop circle (r = {CROP_RADIUS} m)"))
